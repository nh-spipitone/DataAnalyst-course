...
ricavo_mensile_query = text("""
    SELECT DATE_TRUNC('month', data_vendita) :: date AS mese, ROUND (SUM(quantita * prezzo_unitario) :: numeric, 2) AS ricavo_totale FROM vendite GROUP BY mese ORDER BY mese;""")

ricavo_mensile = pd.read_sql(ricavo_mensile_query, engine)
