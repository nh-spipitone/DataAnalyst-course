...
df = pd.read_csv(
    r"FILE DATASET\Travel details dataset PROGETTINO.csv",
    encoding="utf-8",
...
print(
    "Informazioni sulle colonne:", df.info()
)  # Mostra info sulle colonne, tipi di dato, valori nulli
print(
    "Valori mancanti per colonna:\n", df.isnull().sum()
)  # Conta quanti valori mancanti ci sono per colonna
...
def pulizia_costi(value):
    if pd.isnull(value):  # Se il valore è nullo, restituisci None
        ...
        return None
    value = (
        str(value).replace("$", "").replace("USD", "").replace(",", "").replace(" ", "")
    )
    try:
...
)  # errors='coerce' forza i valori non validi a diventare NaT (missing)
df["End date"] = pd.to_datetime(
    df["End date"], errors="coerce"
)  # pd.to_datetime trasforma stringhe in oggetti datetime.
...
df["Costo totale"] = (
    df["Accommodation cost"] + df["Transportation cost"]
)  # Creazione di una nuova colonna "Costo totale"
df["Costi per giorno"] = (
    df["Costo totale"] / df["Duration (days)"]
)  # Creazione di una nuova colonna "Costi per giorno"
...
print(
    "Tipologie di trasporto disponibili:", df["Transportation type"].unique()
)  # unique: funzione per vedere i valori unici
print("Tipologie di alloggio disponibili:", df["Accommodation type"].unique())


df["Transportation type"] = df[
    "Transportation type"
].str.strip()  # Rimuove gli spazi bianchi iniziali e finali
df["Accommodation type"] = df[
    "Accommodation type"
].str.strip()  # Rimuove gli spazi bianchi iniziali e finali
...
def pulizia_stati_destinazioni(destination):
    ...
    return destination.split(",")[
        0
    ].strip()  # Prende solo la prima parte prima della virgola e rimuove spazi extra
...
)  # Conta le occorrenze di ogni destinazione e ordina in ordine decrescente
print(
    "Destinazioni maggiori:", top_destinazioni
)  # Mostra le destinazioni maggiori con il loro numero di occorrenze
...
sns.barplot(
    x=top_destinazioni.values, y=top_destinazioni.index, palette="viridis"
)  # Crea il barplot
...
costo_medio_alloggio = (
    df.groupby("Accommodation type")["Accommodation cost"].mean().sort_values()
)  # Calcola il costo medio per ogni tipo di alloggio
print(
    "Costo medio per tipo di alloggio:", costo_medio_alloggio
)  # Mostra il costo medio per ogni tipo di alloggio


plt.figure(figsize=(10, 6))
sns.barplot(
    x=costo_medio_alloggio.values, y=costo_medio_alloggio.index, palette="magma"
)
...
trasporto_per_età = (
    df.groupby(["Età fascia", "Transportation type"]).size().unstack().fillna(0)
)
...
print(f"Features numeriche: {numerical_features}")
...
# Creiamo e addestriamo lo scaler sui dati di training
scaler = StandardScaler()
X_train_scaled = X_train.copy()
X_test_scaled = X_test.copy()

# Normalizziamo solo le colonne numeriche (identificate prima della codifica)
X_train_scaled[numerical_features] = scaler.fit_transform(X_train[numerical_features])
X_test_scaled[numerical_features] = scaler.transform(X_test[numerical_features])

print("Features normalizzate con StandardScaler!")
print(f"Features normalizzate: {numerical_features}")
...
)  # Calcola la radice quadrata dell'errore quadratico medio
