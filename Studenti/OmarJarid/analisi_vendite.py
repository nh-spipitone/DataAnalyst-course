...
def cerca_prodotto_efficiente(nome_prodotto: str, df: pd.DataFrame):
    for index, row in df.iterrows():
        ...
        if nome_prodotto.lower() == prodotto.lower():
            print(f"Prodotto trovato: {prodotto}")
            print(f"Quantità: {row['quantita']} - Prezzo unitario: {row['prezzo_unitario']}")
...
df_vendite = carica_dati("Esercizi\\Giorno 3\\vendite.csv")
...
while True:
    ...
    if scelta == "1":
        ...
        cerca_prodotto_efficiente(nome_prodotto, df_vendite)
    else:
