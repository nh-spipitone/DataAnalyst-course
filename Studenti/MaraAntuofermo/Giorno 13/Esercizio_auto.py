...
X = df_price[["enginesize", "horsepower", "doornumber", "cylindernumber"]]
print(X.head())
Y = df_price["price"]
...
print(
    "Per ogni porte in meno dell'auto, il prezzo aumenta di circa {:.2f} €.".format(
        coefficients[2]
    )
)

print(
    "Per ogni numero di cilindri in più, il prezzo aumenta di circa {:.2f} €.".format(
        coefficients[1]))
